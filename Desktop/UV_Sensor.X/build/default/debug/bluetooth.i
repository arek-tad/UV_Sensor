...
void BT_ClearCommand(void)
{
    ...
    memset(BTLineBuffer[BTWriteIndex].buf, 0, 0x200);
    BTBufIndex = 0;
}

char* BT_GetLine(void)
{
    static char linebuffer[0x200] = {0};

    if(BTLineBuffer[BTReadIndex].ready)
    {
        memcpy(linebuffer, BTLineBuffer[BTReadIndex].buf, 0x200);
        memset(BTLineBuffer[BTReadIndex].buf, 0, 0x200);
        BTLineBuffer[BTReadIndex].ready = 0;
        BTReadIndex = (BTReadIndex + 1) & 0xF;
        return linebuffer;
    }
...
{
    char command[32];
    uint16_t uvi_scaled = (uint16_t)(UVI * 100);
 BT_EnterCommandMode();
